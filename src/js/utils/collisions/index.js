class Collision {
    ...
    collide(object1, object2) {
        let object1isArray = Array.isArray(object1);
        let object2isArray = Array.isArray(object2);

        this.collisionObject.collide = false;
        this.collisionObject.side = null;

        if (!object1isArray && !object2isArray) {
            this.collideHandler(object1, object2);
        } else if (!object1isArray && object2isArray) {
            for (let i = 0; i < object2.length; i++) {
                this.collideHandler(object1, object2[i]);
            }
        } else if (object1isArray && !object2isArray) {
            for (let i = 0; i < object1.length; i++) {
                this.collideHandler(object1[i], object2);
            }
        } else {
            for (let i = 0; i < object1.length; i++) {
                for (let j = 0; j < object2.length; j++) {
                    this.collideHandler(object1[i], object2[j]);
                }
            }
        }
    ...
    collideHandler(object1, object2) {
        if (!object1 || !object2) {
            return false;
        }

        const vX = (object1.x + (object1.width / 2)) - (object2.x + (object2.width / 2));
        const vY = (object1.y + (object1.height / 2)) - (object2.y + (object2.height / 2));
        const hWidths = (object1.width / 2) + (object2.width / 2);
        const hHeights = (object1.height / 2) + (object2.height / 2);

        if (Math.abs(vX) < hWidths && Math.abs(vY) < hHeights) {
            this.collisionObject.collide = true;
            if (getSide) {
                const oX = hWidths - Math.abs(vX);
                const oY = hHeights - Math.abs(vY);
                if (oX >= oY) {
                    if (vY > 0) {
                        this.collisionObject.side = "top";
                    } else {
                        this.collisionObject.side = "bottom";
                    }
                } else {
                    if (vX > 0) {
                        this.collisionObject.side = "left";
                    } else {
                        this.collisionObject.side = "right";
                    }
                }
            }
        }
...
function update() {
    ...
    for (var i = 0; i < terrain.length; i++) {
        ...
        //var dir = cll.collide(player, terrain[i]);
        // if (dir.collide) {
        //     if (dir.side === "left" || dir === "right") {
        //         player.velX = 0;
        ...
        //     }
        // }
    //    let g = cll.collide(player, terrain[i])
    //    if (g.collide) {
    //     console.log(g);
    //    }
